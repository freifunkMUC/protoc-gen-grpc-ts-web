...
{{range $method := .Method}}
	private methodInfo{{$method.Name}} = new grpcWeb.MethodDescriptor<{{requestMessage $method $file}}, {{responseMessage $method $file}}>(
		"{{$method.Name}}",
		{{if $method.ServerStreaming}}'server_streaming'{{else}}'unary'{{end}},
		{{requestMessage $method $file}} as unknown as MessageCtor<{{requestMessage $method $file}}>,
		{{responseMessage $method $file}} as unknown as MessageCtor<{{responseMessage $method $file}}>,
